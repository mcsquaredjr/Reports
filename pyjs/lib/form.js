...
$pyjs['loaded_modules']['form'] = function (__mod_name__) {
	...
	$m['today']['__name__'] = 'today';

	$m['today']['__bind_type__'] = 0;
	$m['today']['__args__'] = [null,null];
	$m['to_date'] = function(date_time_obj) {

		return date_time_obj['strftime']('%d/%m/%Y');
	};
	$m['to_date']['__name__'] = 'to_date';

	$m['to_date']['__bind_type__'] = 0;
	$m['to_date']['__args__'] = [null,null,['date_time_obj']];
	$m['Form_Row'] = (function(){
	...
	, 1, [null,null,['self']]);
		...
		$method = $pyjs__bind_method2('get_impediment_data', function() {
			...
			var $$comment,end_date,lbl_text,state,ind,start_date,desc;
			...
			ind = lbl_text['find'](':');
			start_date = $p['__getslice'](lbl_text, ind, null);
			if ($p['bool']($p['op_eq'](state, 'Open'))) {
				end_date = null;
			}
			else {
				end_date = $m['today']()['to_date']();
			}
